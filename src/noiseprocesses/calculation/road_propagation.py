...
class RoadPropagationCalculator:
    ...
    def calculate_propagation(
        ...
        try:
            ...
            lden_config.setComputeLDay(not config.output_controls.skip_lday)
            lden_config.setComputeLEvening(not config.output_controls.skip_levening)
            lden_config.setComputeLNight(not config.output_controls.skip_lnight)
            lden_config.setComputeLDEN(not config.output_controls.skip_lden)
            lden_config.setMergeSources(not config.output_controls.export_source_id)
            
            # Initialize noise map calculator
            noise_map = self.java_bridge.PointNoiseMap(
                config.required_tables.building_table,
                config.required_tables.roads_table, 
                config.required_tables.receivers_table
            )
            ...
            if dem_table := config.optional_tables.dem_table:
                noise_map.setDemTable(dem_table)
            if ground_table := config.optional_tables.ground_absorption_table:
            ...
            geom_fields = self.database.query(f"""
                ...
                WHERE f_table_name = '{config.required_tables.receivers_table.upper()}'
            """)
    ...
    def _init_noise_map(self, buildings_table: str, emission_table: str, receivers_table: str):
    ...
    def _set_wind_rose(self, env_data, occurrences_str: str):
        ...
        try:
            ...
            occurrences = [
                float(val.strip())
                for val in occurrences_str.split(',')
            ]
