...
RUN --mount=type=cache,target=$CACHE_DIR git clone --depth 1 --branch 4.0.5-custom-coefficients https://github.com/LGV-Hamburg/NoiseModelling.git NoiseModelling \
    && cd NoiseModelling \
...
RUN ${POETRY_ENV}/bin/poetry version
