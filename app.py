...
class TrafficNoiseProp(BaseProcess):
    ...
    process_description = ProcessDescription(
        ...
        version="v4.0.5custom-coefficients-1.1.2",
        description=(
            ...
            "roads": ProcessInput(
                ...
                schema=Schema(
                    allOf=[
                        ...
                        {"$ref": "https://geojson.org/schema/FeatureCollection.json"},
                        {
                            "$ref": "https://raw.githubusercontent.com/LGV-Hamburg/noiseprocesses/refs/heads/feature/emission-coefficients/schemas/roads-schema.json"
                        },
                    ]
                ),
                minOccurs=1,
...
class TrafficNoiseBuildings(BaseProcess):
    ...
    process_description = ProcessDescription(
        ...
        version="v4.0.5custom-coefficients-1.2.2",
        description=(
            "A process for calculating traffic noise "
...
@register_process("traffic_noise_propagation_hh")
class TrafficNoisePropHh(BaseProcess):
    async def execute(
        self,
        exec_body: Dict[str, Any],
        job_progress_callback: Callable[[int, str], None] | None = None,
    ...
    # Define process description as a class variable
    process_description = ProcessDescription(
        id="traffic_noise_propagation_hh",
        title="Traffic Noise Propagation Calculation",
        ...
        description=(
            ...
            "propagation and creating isosurfaces based on NoiseModelling 4.0.5"
            "(https://noise-planet.org/noisemodelling.html). "
            "Includes german national vehicle types and pavements."
        ),
        jobControlOptions=[
            ProcessJobControlOptions.ASYNC_EXECUTE,
        ],
        ...
        inputs={
            ...
            "emission_coefficients_profile": ProcessInput(
                ...
                description=(
                    "The emission coefficients profile to use. "
                    "Default is HH which defines the core "
                    "emission parameters for the CNOSSOS-EU (Common Noise Assessment "
                    "Methods in Europe) road traffic noise model, "
                    "specifically the 2020 amendments and adds german national "
                    "coefficients and pavement types based."
                ),
                schema=Schema(
                    type="string",
                    enum=["cnossos2015", "cnossos2020", "hh"],
...
@register_process("traffic_noise_buildings_hh")
class TrafficNoiseBuildingsHh(BaseProcess):
    async def execute(
        self,
        exec_body: Dict[str, Any],
        job_progress_callback: Callable[[int, str], None] | None = None,
        ...
        inputs={
            ...
            "emission_coefficients_profile": ProcessInput(
                ...
                description=(
                    ...
                    "emission parameters for the CNOSSOS-EU (Common Noise Assessment "
                    "Methods in Europe) road traffic noise model, "
                    "specifically the 2020 amendments and adds coefficients for "
                    "german national vehicle types and pavements."
                ),
                schema=Schema(
                    type="string",
                    enum=["cnossos2015", "cnossos2020", "hh"],
